...
export default function ArchiveView({ ads, onSelectAd }: Props) {
  ...
  return (
    <div className="flex-1 p-6 md:p-8 overflow-y-auto max-w-[1200px] mx-auto w-full">
      <div className="mb-6">
        <h2 className="text-2xl font-black text-gray-100">Archive</h2>
        <p className="text-gray-500 text-sm font-medium mt-0.5">All Winner and killed ads</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="bg-[#141416] border border-gray-800 rounded-2xl px-4 py-3 text-center shadow-sm">
          <p className="text-[8px] font-black uppercase text-gray-500 tracking-widest mb-1">Total Archived</p>
          <p className="text-xl font-black text-gray-100">{winnerCount + killedCount}</p>
        </div>
        <div className="bg-green-950/30 border border-green-900 rounded-2xl px-4 py-3 text-center shadow-sm">
          <p className="text-[8px] font-black uppercase text-green-500 tracking-widest mb-1">Winner</p>
          <p className="text-xl font-black text-green-400">{winnerCount}</p>
        </div>
        <div className="bg-red-950/30 border border-red-900 rounded-2xl px-4 py-3 text-center shadow-sm">
          <p className="text-[8px] font-black uppercase text-red-500 tracking-widest mb-1">Killed</p>
          <p className="text-xl font-black text-red-400">{killedCount}</p>
        </div>
        <div className="bg-blue-950/30 border border-blue-900 rounded-2xl px-4 py-3 text-center shadow-sm">
          <p className="text-[8px] font-black uppercase text-blue-500 tracking-widest mb-1">Hit Rate</p>
          <p className="text-xl font-black text-blue-400">{hitRate}%</p>
        </div>
      </div>
      ...
      {killedCount > 0 && (
        <div className="bg-red-950/30 border border-red-900 rounded-2xl p-4 mb-6 flex items-center gap-3">
          ...
          <p className="text-sm font-bold text-red-400">Killed ads are automatically deleted 30 days after being killed.</p>
        </div>
      )}

      {/* Search + Filters */}
      <div className="bg-[#141416] border border-gray-800 rounded-2xl p-5 mb-6 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          {(search || filterStatus !== "All" || filterFormat !== "All" || filterResult !== "All") && (
            <button onClick={() => { setSearch(""); setFilterStatus("All"); setFilterFormat("All"); setFilterResult("All"); }} className="text-[10px] font-black text-gray-500 hover:text-red-400 transition-colors uppercase tracking-widest">
              Clear Filters
            </button>
          )}
        </div>
      ...
      <p className="text-[10px] font-black uppercase text-gray-500 tracking-widest mb-4">{archivedAds.length} ads found</p>

      {archivedAds.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-700">
          <div className="text-6xl mb-4">📦</div>
          <p className="text-lg font-bold text-gray-500">No archived ads found</p>
          <p className="text-sm font-medium text-gray-600">Try adjusting your filters</p>
        </div>
      ) : (
          ...
          {archivedAds.map(ad => {
            ...
            return (
              <div
                ...
                className={`bg-[#141416] border rounded-2xl overflow-hidden cursor-pointer hover:shadow-md transition-all ${
                  deletingSoon ? "border-red-800" :
                  isWinner ? "border-green-900 hover:border-green-800" :
              ...
              >
                ...
                <div className={`h-1 ${isWinner ? "bg-green-500" : "bg-red-500"}`} />
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <p className="font-black text-gray-100 leading-snug">{ad.concept_name}</p>
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-full ${isWinner ? "bg-green-950 text-green-400" : "bg-red-950 text-red-400"}`}>
                        {ad.status}
                      </span>
                      {ad.result && (
                        <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-full ${
                          isWinner ? "bg-amber-950 text-amber-400" :
                          isLoser ? "bg-[#1f1f23] text-gray-400" :
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    ...
                    {ad.assigned_editor && (
                      <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-green-950 text-green-400 rounded-md">{ad.assigned_editor}</span>
                    )}
                  </div>
